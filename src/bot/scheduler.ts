    ...
    if (res.success) {
      if (res.position) {
        ...
        const features: TradeFeatures = {
          ...
          confidence: sig.confidence.score,
          rsi: sig.ind.rsi ?? 50,
          macdHistogram: sig.ind.macdHistogram ?? 0,
          adxValue: sig.ind.adxValue ?? 0,
          atrPercent: sig.risk.atrPercent ?? (sig.risk.atr ? (sig.risk.atr / sig.risk.entryPrice) * 100 : 1),
          bbPercent: sig.ind.bbPercent ?? 50,
          ema20rel: sig.ind.ema20 != null ? sig.ind.ema20 / sig.risk.entryPrice : 1,
          ema50rel: sig.ind.ema50 != null ? sig.ind.ema50 / sig.risk.entryPrice : 1,
          ema200rel: sig.ind.ema200 != null ? sig.ind.ema200 / sig.risk.entryPrice : 1,
          volumeAbove: sig.ind.volumeSignal === 'above_avg' ? 1 : 0,
