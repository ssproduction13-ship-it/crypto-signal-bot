import { loadJournal, loadWeights } from "./storage.js";
  ...
  export async function buildSelfAnalysis(chatId?: number): Promise<string> {
    ...
    const closed = (chatId!=null ? all.filter(e=>e.chatId===chatId) : all)
                     .filter(e=>e.closedAt);
    ...
    const recent = closed.slice(-100);
    const wins   = recent.filter(e=>(e.pnlPercent??0)>0);
    const losses = recent.filter(e=>(e.pnlPercent??0)<=0);
    const wr     = (wins.length/recent.length*100);
    const avgW   = wins.length  ? wins.reduce((a,e)=>a+(e.pnlPercent??0),0)/wins.length   : 0;
    ...
    const ret    = recent.map(e=>e.pnlPercent??0);
    const sp     = sharpe(ret);
    ...
    for (const e of recent) {
      ...
      bySym[e.symbol]!.t++; bySym[e.symbol]!.pnl+=e.pnlPercent??0;
      if((e.pnlPercent??0)>0) bySym[e.symbol]!.w++;
    }
    ...
    return [
      `📊 *Самоанализ (последние ${recent.length} сделок)*`, "",
