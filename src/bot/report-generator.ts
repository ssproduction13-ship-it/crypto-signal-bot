...
// ── Decision Log section builder ─────────────────────────────────────────────

function buildDecisionLogSection(log: DecisionTrace[], stats: Awaited<ReturnType<typeof getDecisionStats>>): string {
  const opened   = log.filter(d => d.verdict === "OPEN");
  const rejected = log.filter(d => d.verdict === "REJECT");
  ...
  if (log.length === 0) {
    return statsHtml + '<div class="empty">Нет записей в Decision Log</div>';
  }

  // Show last 10 decisions with full trace
  const recent = log.slice(0, 10);

  const rows = recent.map(d => {
    const icon = d.verdict === "OPEN" ? "✅" : "❌";
    const time = new Date(d.timestamp).toLocaleString("ru-RU", {
      month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit"
    });

    // Parse strategy ranking from "Выбор стратегии" step
    ...
    // Parse other strategies from note field
    const otherStrats = rankNote
      ? rankNote.split(" | ").map(part => {
    ...
    const stepsHtml = d.steps.length ? `
      <details style="margin-top:8px">
        <summary style="font-size:12px;color:#64748b;cursor:pointer">
          ${passCount} ✅ / ${failCount} ❌ шагов — показать детали
        </summary>
        <table style="margin-top:6px;font-size:12px">
          <tr><th>Проверка</th><th>Результат</th><th>Значение</th></tr>
          ${d.steps.map(s => {
            const si = s.result === "PASS" ? "✅" : s.result === "FAIL" ? "❌" : "⏭";
            const val = s.value !== undefined ? String(s.value) : "";
            const thr = s.threshold !== undefined ? ` / мин ${s.threshold}` : "";
            return `<tr>
              <td>${esc(s.check)}</td>
              <td>${si}</td>
              <td style="font-size:11px;color:#94a3b8">${esc(val + thr)}${s.note && s.result !== "PASS" ? ` — ${esc(s.note)}` : ""}</td>
            </tr>`;
          }).join("")}
        </table>
      </details>` : "";

    const rankingHtml = rankStep ? `
      <div style="margin-top:6px;font-size:12px">
        <span style="color:#64748b">🏆 Победитель: </span>
        <strong style="color:#34d399">${esc(winnerVal)}</strong>
        ${otherStrats.length ? `
        <div style="margin-top:4px;color:#64748b">Прочие: ${
          otherStrats.map(s => `${s!.strategy} (score=${s!.score}, trust=${s!.trust}, w=${s!.weight}%)`).join(" · ")
        }</div>` : ""}
      </div>` : "";
...
function buildAISummary(
  ...
  if (stats && stats.wr >= 50)
    items.push(`<li>✅ <span class="highlight">WR ${stats.wr.toFixed(1)}%</span> — выше 50%, хороший результат</li>`);
  ...
  if (stats && stats.wr < 40)
    items.push(`<li>⚠️ <span class="warn-text">WR ${stats.wr.toFixed(1)}%</span> — ниже нормы</li>`);
  ...
  if (leadStrat && leadStrat.trades > 0)
    items.push(`<li>🏆 Лидирует: <span class="highlight">${leadStrat.strategy}</span> — PF ${leadStrat.profitFactor >= 999 ? "∞" : leadStrat.profitFactor.toFixed(2)}, WR ${leadStrat.winRate.toFixed(1)}%</li>`);
  if (degradStrat && degradStrat.trades > 0 && degradStrat.strategy !== leadStrat?.strategy)
    items.push(`<li>📉 Слабейшая: <span class="warn-text">${degradStrat.strategy}</span> — PF ${degradStrat.profitFactor >= 999 ? "∞" : degradStrat.profitFactor.toFixed(2)}</li>`);

  // Decision log insights
  if (d.decisionStats.total > 0) {
    const openRate = d.decisionStats.total > 0 ? (d.decisionStats.opened / d.decisionStats.total * 100).toFixed(1) : "0";
    items.push(`<li>📊 Trade Quality Gate: принято <span class="highlight">${openRate}%</span> сигналов из ${d.decisionStats.total} за 7 дней</li>`);
    if (d.decisionStats.topRejectReasons[0])
      items.push(`<li>🚫 Главная причина отказов: <span class="warn-text">${esc(d.decisionStats.topRejectReasons[0].reason)}</span> (${d.decisionStats.topRejectReasons[0].count} раз)</li>`);
  }
  ...
  items.push(`<li>💡 Рекомендация: ${
    d.closedTrades.length < 20
      ...
      : dd > 15
      ? "Снизь riskPercent до 0.5% для уменьшения просадки"
...
function buildHtml(d: ReportData): string {
  ...
  const aiSummary = buildAISummary(d, stats, health, readiness, stratsSorted[0], stratsSorted[stratsSorted.length - 1], dd);
...
.progress-note{font-size:11px;color:#475569;width:140px;text-align:right;flex-shrink:0}
...
<div class="kpi-grid">
  ...
  <div class="kpi"><div class="label">Общий P&L</div>
    ...
    <div class="sub">${pnlSign(d.balance - d.initialBalance)}$${fmt(Math.abs(d.balance - d.initialBalance))}</div></div>
  <div class="kpi"><div class="label">WinRate</div>
    <div class="value ${(stats?.wr ?? 0) >= 50 ? "pos" : (stats?.wr ?? 0) >= 40 ? "warn" : "neg"}">${fmt(stats?.wr ?? 0)}%</div>
    <div class="sub">${stats?.wins.length ?? 0}W / ${stats?.losses.length ?? 0}L</div></div>
  ...
  <div class="kpi"><div class="label">Готовность</div>
    ...
    <div class="sub">для реальной торговли</div></div>
</div>
...
<details open>
  ...
  <div class="section-body">
    ${stats ? `<table>
      ...
      <tr><td>WinRate</td><td class="${stats.wr >= 50 ? "pos" : stats.wr >= 40 ? "warn" : "neg"}">${fmt(stats.wr)}%</td><td class="${stats.wr >= 45 ? "pos" : "warn"}">${stats.wr >= 45 ? "✅" : "⚠️ нужно 45%+"}</td></tr>
      <tr><td>Profit Factor</td><td class="${stats.pf >= 1.3 ? "pos" : stats.pf >= 1 ? "warn" : "neg"}">${stats.pf >= 999 ? "∞" : fmt(stats.pf)}</td><td class="${stats.pf >= 1.3 ? "pos" : "warn"}">${stats.pf >= 1.3 ? "✅" : "⚠️ нужно 1.3+"}</td></tr>
...
<details open>
  ...
  <div class="section-body">
    <table>
      ...
      ${d.strategyStats.map(s => {
        ...
        return `<tr>
          ...
          <td class="${s.profitFactor >= 1.3 ? "pos" : s.profitFactor >= 1 ? "warn" : "neg"}">${s.profitFactor >= 999 ? "∞" : fmt(s.profitFactor)}</td>
    ...
    <hr class="divider">
    <div class="section-title">A/B Тест</div>
...
<details>
  ...
  <div class="section-body">
    <div class="kpi-grid">
      <div class="kpi"><div class="label">Обучающих сделок</div><div class="value">${d.closedTrades.length}</div><div class="sub">Из 100 для полного цикла</div></div>
      <div class="kpi"><div class="label">Чемпион A/B</div><div class="value" style="font-size:16px">${champion?.name ?? "Определяется"}</div><div class="sub">${champion ? `Сделок: ${champion.trades}` : "Нужно 20+ сделок"}</div></div>
    </div>
    <hr class="divider">
    <div class="section-title">Веса факторов</div>
    ...
    <table>
      ...
      ${d.abVariants.map(v => {
        ...
        return `<tr><td>${v.name}</td><td>${v.trades}</td><td>${wr}%</td><td>${pfl}</td>
          <td>${v.isChampion ? "👑 Чемпион" : v.isActive ? "🎯 Активный" : "⏳ Тест"}</td></tr>`;
      }).join("")}
    </table>
  </div>
</details>
...
<details>
  ...
  <div class="section-body">
    <div style="text-align:center;margin-bottom:16px">
      ...
      <div style="color:#64748b;font-size:13px">${readiness.total >= 70 ? "✅ Близко к готовности" : readiness.total >= 40 ? "⚠️ Нужно больше данных" : "❌ Продолжай бумажную торговлю"}</div>
    </div>
  ...
  <div class="section-body">
    ...
    <table>
      ...
      <tr><td>WinRate</td>
        ...
        <td class="${(health.sAll?.wr ?? 0) >= 45 ? "pos" : "warn"}">${health.sAll ? fmt(health.sAll.wr) + "%" : "—"}</td></tr>
      <tr><td>Profit Factor</td>
        ...
        <td class="${(health.sAll?.pf ?? 0) >= 1.3 ? "pos" : (health.sAll?.pf ?? 0) >= 1 ? "warn" : "neg"}">${health.sAll ? (health.sAll.pf >= 999 ? "∞" : fmt(health.sAll.pf)) : "—"}</td></tr>
      <tr><td>Expectancy</td>
        ...
        <td class="${(health.sAll?.expectancy ?? 0) >= 0 ? "pos" : "neg"}">${health.sAll ? pnlSign(health.sAll.expectancy) + fmt(health.sAll.expectancy) + "%" : "—"}</td></tr>
    </table>
  </div>
</details>
...
<details open>
  <summary>🔬 Decision Log — путь принятия решений AI</summary>
  <div class="section-body">
