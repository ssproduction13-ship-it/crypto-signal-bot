...
async function buildAccountStats(chatId: number): Promise<string> {
  ...
  return [
    ...
    `📂 Открытых (${account.positions.length}/5):`,
    ...posLines,
  ].join("\n");
}
...
export function createBot(): Telegraf {
  ...
  bot.command("status", async (ctx) => {
    ...
    await ctx.reply(
      ...
      `📂 Открыто позиций: *${account.positions.length}/5*\n` +
  ...
  // /report — generate and send HTML daily report on demand
  bot.command("report", async (ctx) => {
    ...
    try {
      ...
      await ctx.reply("❌ Ошибка генерации отчёта. Попробуй позже.");
    }
