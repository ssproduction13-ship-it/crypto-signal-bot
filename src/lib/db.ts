...
// Pool creation is lazy — no connection attempt until pool.connect()
// So missing DATABASE_URL does NOT crash the process at import time.
...
export async function initDb(): Promise<void> {
  if (!process.env["DATABASE_URL"]) {
    throw new Error(
      "DATABASE_URL is not set. In Railway: service Variables -> New Variable -> " +
